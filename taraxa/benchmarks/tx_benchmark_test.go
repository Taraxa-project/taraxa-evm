...
func BenchmarkRoot(b *testing.B) {
	benchmarking.AddBenchmark(b, "single_coin_tx", func(name string) benchmarking.Benchmark {
		test_amount := new(big.Int).SetUint64(math.MaxUint64)
		...
		return func(id int, b *testing.B) {
			b.StopTimer()
			...
			defer func() {
				defer raw_db.Close()
				util.PanicIfNotNil(os.RemoveAll(db_path))
			}()
			...
			db := state.NewDatabaseWithCache(raw_db, 500)
