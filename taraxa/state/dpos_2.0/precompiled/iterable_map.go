...
func (self *IterableMap) Init(stor StorageWrapper, prefix []byte) {
	self.storage = stor
	self.accounts_storage_prefix = append(prefix, bin.BytesView("accounts")...)
	self.accounts_count_storage_key = stor_k_1(prefix, bin.BytesView("accounts_count"))
	self.accounts_pos_storage_prefix = append(prefix, bin.BytesView("accounts_pos")...)
}
...
func (self *IterableMap) GetAccounts(batch uint32, count uint32) (result []common.Address, end bool) {
	...
	if requested_idx_start >= accounts_count {
		panic("Invalid batch provided - there is not so many accounts in iterable map")
	}
	...
	for idx = uint32(requested_idx_start + 1); idx <= requested_idx_end && idx <= accounts_count; idx++ {
		//for idx := uint32(1); idx <= accounts_count; idx++ {
