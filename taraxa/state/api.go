...
//	self.state_transition_service.Init(db, disable_block_rewards, last_root_hash, chain_cfg, execution_opts, last_blk_num, get_block_hash, )
//	return self
//}
...
func (self *API) BeginConcurrentSchedule(target_blk *vm.Block) {
	self.concur_schedule_generation = ConcurrentScheduleGeneration{
		&self.state_transition_service.db,
...
func (self *API) DryRun(blk *vm.Block, trx *vm.Transaction, opts vm.ExecutionOptions) (ret vm.ExecutionResult) {
	...
	evm_state := NewEVMState(&BlockDB{self.db, blk.Number}, EvmStateOpts{
		AccountCacheSize:      32,
...
func (self *API) GetBlockState(blk_num types.BlockNum) (ret BlockDB) {
	ret = BlockDB{self.db, blk_num}
	return
}
