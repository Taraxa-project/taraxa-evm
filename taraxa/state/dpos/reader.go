...
func (self *Reader) Init(cfg *Config, blk_n types.BlockNum, without_delay bool, storage_factory func(types.BlockNum) storage.StorageReader) *Reader {
	self.cfg = cfg
	...
	self.storage = new(storage.StorageReaderWrapper).Init(contract_address, storage_factory(blk_n_actual))
	return self
}
...
func (self Reader) GetStakingBalance(addr *common.Address) (ret *big.Int) {
	ret = bigutil.Big0
	self.storage.Get(storage.Stor_k_1(field_staking_balances, addr[:]), func(bytes []byte) {
		ret = bigutil.FromBytes(bytes)
	})
	return
}
		...
		for i := 0; i < 2; i++ {
			...
			self.storage.ListForEach(bin.Concat2(list_kind, addr[:]), func(addr_other_raw []byte) {
				...
				if !addrs_only {
					...
					self.storage.Get(storage.Stor_k_1(field_deposits, addr1[:], addr2[:]), func(bytes []byte) {
						var deposit Deposit
