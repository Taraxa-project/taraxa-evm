...
type BlockPostProcessor struct {
	...
	outbox chan *api.StateTransitionReceipt
}

func LaunchBlockPostProcessor(block *api.Block, newStateDB StateDBFactory, onErr util.ErrorHandler) *BlockPostProcessor {
	inbox := make([]chan *TransactionResultWithStateChange, len(block.Transactions))
	outbox := make(chan *api.StateTransitionReceipt, 1)
	for txId := range block.Transactions {
		inbox[txId] = make(chan *TransactionResultWithStateChange, 1)
	}
	go func() {
		result := new(api.StateTransitionReceipt)
		...
		for txId, channel := range inbox {
			...
			result.Receipts = append(result.Receipts, &api.TaraxaReceipt{
...
func (this *BlockPostProcessor) AwaitResult() (ret *api.StateTransitionReceipt, ok bool) {
	ret, ok = <-this.outbox
	return
}
